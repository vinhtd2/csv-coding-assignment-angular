...
export class BackendService {
  ...
  tasks() {
    return of(this.storedTasks.filter(task => task.completed === false)).pipe(delay(randomDelay()));
  }

  task(id: number): Observable<Task> {
    console.log('task', of(this.findTaskById(id)).pipe(delay(randomDelay())))
  ...
  newTask(description) {
    ...
    console.log('this.storedTasks', this.storedTasks)
